...
export function KeywordsTab() {
  ...
  const handleDelete = async (template: OCRTemplate) => {
    if (!window.confirm(`确认删除「${template.name}」？`)) return;
    try {
      await deleteKeywordTemplate(template.id);
      toast.success('已删除');
      load();
    } catch (error: any) {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-muted-foreground">
        关键词模版在 OCR 检测页面中使用。可在检测页面设置面板中将当前配置保存为模版，或从此处查看和删除。
      </p>
      {loading && <div className="py-8 text-center text-muted-foreground">加载中...</div>}
      {!loading && templates.length === 0 && (
        <div className="py-12 text-center text-muted-foreground">暂无关键词模版</div>
      )}
          ...
          >
            <div className="min-w-0 flex-1">
              ...
              <div className="mt-1.5 flex flex-wrap gap-1">
                {template.keywordConfigs.map((keyword, index) => (
                  <span
                    key={index}
