...
# HTTPS前端 + HTTP后端生产环境启动脚本
# 前端HTTPS (443) -> 后端HTTP (8012)

echo "🚀 启动HTTPS前端 + HTTP后端生产环境..."
...
with socketserver.TCPServer(('0.0.0.0', PORT), Handler) as httpd:
    ...
    print('🔗 API代理: /api/* -> http://localhost:8012/api/*')
    print('按 Ctrl+C 停止服务')
    httpd.serve_forever()
...
echo "   - 后端API: http://$LOCAL_IP:8012/api/"
echo ""
echo "🔗 API代理配置:"
